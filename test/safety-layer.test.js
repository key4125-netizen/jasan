...
// [FUTURE-P1 Phase 2-C] 절세계좌가 별도 범위로 함께 계산되면서 "절세계좌는 제외된다"는 예전 문장은
// 사실이 아니게 됐다 - 문구가 실제 계산 범위와 어긋나지 않는지를 이 테스트가 고정한다.
test('explainAccumulationScopeAlwaysOn - MC 계산 범위(화면 위 숫자=일반계좌, 절세계좌=별도 계산, 부동산·공동=제외)를 안내한다', () => {
  const issue = explainAccumulationScopeAlwaysOn();
  assert.strictEqual(issue.severity, SAFETY_LEVEL.INFO);
  assert.ok(issue.message.includes('일반계좌'), '화면 위 숫자의 기준(일반계좌)을 명시해야 한다');
  assert.ok(issue.message.includes('절세계좌'), '절세계좌를 어떻게 다루는지 명시해야 한다');
  assert.ok(issue.message.includes('부동산'), '제외 대상(부동산)을 명시해야 한다');
  assert.ok(issue.message.includes('공동'), '제외 대상(공동 자산)을 명시해야 한다');
  // 실제 계산은 절세계좌를 별도 범위로 포함하므로, 예전의 "절세계좌는 계산에서 제외" 문구가
  // 남아 있으면 안 되고, 어떻게 다루는지(따로 계산)가 대신 적혀 있어야 한다.
  assert.ok(!issue.message.includes('절세계좌·부동산·공동 자산은 계산에서 제외'), '실제 계산과 어긋나는 옛 문구가 남아 있다');
  assert.ok(issue.message.includes('따로 계산'), '절세계좌를 별도로 계산한다는 사실이 빠졌다');
