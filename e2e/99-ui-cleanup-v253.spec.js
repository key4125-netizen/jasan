...
//   B. 시장 현황 & 매크로 브리핑: 「📌 세부 내용 보기」 → 「📄 상세 현황 보기」(펼침/접힘 동작 그대로).
...
test('B. 매크로 브리핑의 접기 버튼 이름이 「📄 상세 현황 보기」이고 펼침/접힘이 그대로 동작한다', async ({ page }) => {
  await boot(page);
  await seedMacro(page);
  const btn = page.locator('#macroDiagnosisToggleBtn');
  await expect(btn).toHaveText('📄 상세 현황 보기');
  expect(await page.locator('#macroBriefingSection').innerText()).not.toContain('세부 내용 보기');
  const bodyHeight = () => page.locator('#macroDiagnosisBody').evaluate((el) => el.getBoundingClientRect().height);
...
for (const w of [375, 1440]) {
  for (const dark of [false, true]) {
    test(`E. ${w}px ${dark ? 'Dark' : 'Light'} - 바뀐 표시가 14px 이상이고 잘리거나 넘치지 않는다`, async ({ page }) => {
      ...
      const macroLabel = await fit(page.locator('#macroDiagnosisToggleBtn span'));
      expect(macroLabel.fs).toBeGreaterThanOrEqual(14);
      expect(macroLabel.clipped).toBeLessThanOrEqual(1);
      expect((await fit(page.locator('#macroDiagnosisToggleBtn'))).h).toBeGreaterThanOrEqual(44);
