// E2E-83 [신규 시작 안정화] 데이터 초기화 → 다음 날 새 자산·거래 입력 → Day1 / Day2(거래 없음) / Day3(매수)에서
...
//   - 데이터 초기화가 메모리의 티커 역할·학습 종목명을 남겨, 재실행 전에 자산을 추가하면 예전 역할이 다시 저장됐다(js/14).
...
test('N3. 데이터 초기화는 오늘 0원 기록을 남기지 않고, 메모리의 티커 역할·학습 종목명도 비워 재실행 전 입력에 섞이지 않는다', async ({ page }) => {
  await cleanOpen(page);
  page.on('dialog', (d) => d.accept());
...
for (const op of ['pull', 'push']) {
  test(`N6-${op}. 동기화 ${op === 'pull' ? '받기' : '올리기'}가 진행 중일 때 데이터 초기화를 눌러도 예전 데이터가 되살아나거나 Cloud에 올라가지 않는다`, async ({ page, context }) => {
    const kv = await fakeKv(context);
    await cleanOpen(page);
    page.on('dialog', (d) => d.accept());
