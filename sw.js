...
const CACHE_NAME = 'smart-asset-manager-v71'; // [무티커 자산 금액 기반 입력 + 잔고조정 거래 자동생성]
// 거래추가/최초등록/자산수정 모달에 "금액으로 입력" 토글 추가 - 현금/외화/부동산처럼 티커 없는 자산은
// 수량×단가 대신 총 보유금액 하나로 입력할 수 있다(달러는 참고용 적용환율도 함께 기록). 자산 수정
// 모달에서 무티커 자산의 수량/금액을 직접 고치면 그 차액만큼 origin:'adjust' 거래를 거래내역에 자동
// 기록해, 화면에 보이는 잔고와 거래내역 장부가 항상 일치하도록 했다. v70->v71: 이 값을 바꿔야 PWA가
// 캐시해 둔 예전 index.html을 버리고 새 index.html을 다시 받아온다 - 안 바꾸면 GitHub에 새 index.html을
