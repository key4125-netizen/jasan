...
const CACHE_NAME = 'smart-asset-manager-v130'; // [매크로 팝업 - 실제 5일/20일 추세 반영 + 라벨 수정]
// (1) 매크로 지표 상세 팝업의 "📈 최근 시장 동향"이 사실은 하루짜리 등락률이었던 착시를 고쳤다 -
// getCachedDailyCloses(1년치 일별 종가, RISK 엔진/종목 분석과 동일 캐시)로 실제 5일/20일 등락률을
// 계산해(fetchMacroShortTermTrend) "최근 5일간 -1.4%로 박스권, 20일 기준 +0.8%" 같은 진짜 추세
// 문장을 만들고, 오늘 하루가 그 추세와 반대 방향이면("다만 오늘은 반등/조정") 짚어준다. 팝업을 열
// 때만 온디맨드로 조회하고(5분 자동 갱신 주기에 얹지 않음), 한 번 불러온 값은 다음 갱신에도 보존된다.
// (2) 매크로 브리핑 하단 요약 라벨을 "📌 핵심 원인"->"📌 시장 종합 평가"로 수정했다(뒤따르는 문장이
// 여러 지표를 종합한 평가라 원인 하나만 짚는 라벨과 안 맞았음). js/10이 바뀌었으므로 v129->v130으로
