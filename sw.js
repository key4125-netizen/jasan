...
const CACHE_NAME = 'smart-asset-manager-v150'; // [버그 수정 - 최초 접속 시 시세조회가 느려지고 실패하던 문제]
// v149에서 "자동 갱신이 아예 시작 안 됨" 문제를 고쳤더니, 이번엔 "시작은 하는데 매우 느리고 결과를
// 제대로 못 받아온다"는 후속 신고가 들어왔다(재조회를 눌러도 마찬가지) - 원인은 종목 마스터 데이터
// (data/ticker-master.json, 국내 코스피/코스닥 전종목+미국 나스닥/뉴욕/아멕스 주요종목 약 16,000건,
// 약 2.7MB)였다. loadTickerMaster()가 부팅 초반부터 refreshPricesAndRates()와 동시에 네트워크를
// 타면서, 이 큰 파일이 모바일 회선에서 가격 조회 요청들과 대역폭/커넥션을 다퉈 전체적으로 느려지고
// 일부는 타임아웃으로 실패한 것으로 보인다(재조회까지 느렸던 건, 이 최초 경합 때문에 공유 CORS
// 프록시 쪽에서 일시적으로 rate-limit이 걸렸을 가능성).
// [수정] localStorage 캐시 반영(동기, 네트워크 없음)은 그대로 부팅 초반에 하되, 네트워크로 새로 받아야
// 하는 경우만 가격 조회가 끝난 뒤로 미뤘다(js/14) - backfillAllHoldingsDailyPnlHistory와 똑같은 이유로
// 똑같은 자리에 둠(같은 프록시 풀 경합을 피하기 위해 가격 조회를 항상 우선시킨다는 기존 원칙 그대로
// 적용).
// js/14가 바뀌었으므로 v149->v150으로 올려 PWA가 캐시된 예전 버전을 버리고 새로 받아오게 한다.
const APP_SHELL = [
  './',
  './index.html',
  './manifest.json',
