...
const CACHE_NAME = 'smart-asset-manager-v136'; // [신규 기능 - 한국투자증권(KIS) 재무/수급 데이터 연동]
// 국내주식 한정으로 PER/PBR/ROE/EPS/BPS/시가총액/매출·영업이익·순이익(전년비)/부채비율과, 외국인·기관
// 5일·20일 순매수 수급 동향을 종목 상세 모달과 종목 분석 모달 양쪽에 사실 나열 형태로 추가했다(투자
// 판단·추천은 전혀 섞지 않음). 데이터는 사용자가 직접 배포한 cloudflare-worker-kis-proxy.js(신규 Worker,
// KIS 앱키/시크릿은 서버 사이드에만 있고 클라이언트 코드에는 없음) 경유로 받아오며, 국내주식이 아니거나
// 티커가 없는 자산(해외 종목, 부동산/현금 등)이면 영역 자체가 보이지 않는다. 새 파일 js/13-fundamental-
// data.js가 추가되면서 기존 13-settings-boot.js는 14번으로 밀렸다(bootApp()을 마지막에 호출하는 파일이라
// 항상 가장 마지막 번호여야 함). js가 여러 개 바뀌었으므로 v135->v136로 올려 PWA가 캐시된 예전 버전을
// 버리고 새로 받아오게 한다.
const APP_SHELL = [
  './',
  './index.html',
