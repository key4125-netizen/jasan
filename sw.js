...
const CACHE_NAME = 'smart-asset-manager-v143'; // [UI 정돈 - 매크로 브리핑 타이포그래피를 RISK 카드와 통일]
// (1) "시장 종합 평가/내 포트폴리오 영향/대응 가이드" 3줄이 text-lg(18px)라 바로 아래 RISK 카드의
// 같은 성격 텍스트(권장 행동 지침, text-sm/14px)보다 눈에 띄게 크게 보이던 걸 text-sm으로 맞췄다.
// (2) "시장 현황 & 매크로 브리핑" 타이틀을 text-xs(12px)에서 text-sm(14px)으로 올려, RISK 관리/국내
// 자산 Top 5 카드 타이틀과 크기를 통일했다(각 카드 고유의 색상은 유지 - 크기/굵기/여백만 통일).
// index.html/js/10이 바뀌었으므로 v142->v143로 올려 PWA가 캐시된 예전 버전을 버리고 새로 받아오게 한다.
const APP_SHELL = [
  './',
  './index.html',
  './manifest.json',
