...
const CACHE_NAME = 'smart-asset-manager-v108'; // [상단 헤더 및 액션 버튼 레이아웃 재배치]
// 모바일에서 타이틀 블록을 압축해 우측 공간을 확보하고, 그 자리(1번/2번 위치)에 [🔄 시세 & 환율 갱신]
// 과 [📈 핵심종목 실시간]을 세로로 쌓아 배치했다 - 기존 메인 액션 버튼 그룹에서 이 둘을 빼내 타이틀
// 옆으로 옮긴 것뿐이라 id/클릭 핸들러는 그대로다. 남은 6개 버튼(최초등록/엑셀 내보내기/엑셀 업로드/
// JSON 백업/JSON 불러오기/데이터 초기화)은 모바일에서 grid-cols-3로 3개씩 2줄, 데스크탑에서는 기존
// 처럼 한 줄 flex로 배치한다.
// v107->v108: 이 값을 바꿔야 PWA가 캐시해 둔 예전 index.html을 버리고 새 index.html을 다시 받아온다 - 안
