...
const CACHE_NAME = 'smart-asset-manager-v40'; // [아코디언 구조 개편 + 마무리 3종] 자산관리 카드를
// 전체/소유자별/국내해외별/자산군별 4개의 독립적인 아코디언으로 재구성(요약정보·검색필터는 상단 고정),
// 종목별 리밸런싱 실행가이드 카드를 전체/소유자별 순차 아코디언으로 재구성(설명·엑셀다운로드는 상단
// 고정), 달러자산총액/적용환율 카드의 텍스트 클릭 시 팝업 열리던 동작 제거, 상단 헤더 환율뱃지/환율보기/
// 다크모드 버튼이 모바일에서도 한 줄에 배치되도록 반응형 압축.
// v39->v40: 이 값을 바꿔야 PWA가 캐시해 둔 예전 index.html을 버리고 새 파일을 다시 받아온다 - 안 바꾸면
