...
const CACHE_NAME = 'smart-asset-manager-v54'; // [기간별 실현손익 아코디언 + 시나리오 카드 금융/부동산 구분]
// ①"기간별 실현 손익" 표를 <table> 요약에서 행별 아코디언 리스트로 바꿨다 - 일/월/연 각 기간 행을
// 펼치면 그 안에 속한 개별 매도 거래(거래일자/종목명/소유자·계좌구분/수량/단가/실현손익)가 바로 아래로
// 펼쳐진다(renderPnlReportList/togglePnlPeriodDetail). ②미래예측 탭 4개 시나리오 카드의 "20년 후
// 예상자산" 아래에 금융자산/부동산 세부 금액을 추가 표기했다(부동산 미보유 시엔 기존처럼 총액만 표시) -
// 각 시나리오의 최종 스냅샷에 이미 있던 그룹별 잔액(부동산 포함)을 그대로 읽어와 총액에서 뺀 것뿐이라
// 계산 로직 자체는 바뀌지 않았다.
// v53->v54: 이 값을 바꿔야 PWA가 캐시해 둔 예전 index.html을 버리고 새 파일을 다시 받아온다 - 안 바꾸면
