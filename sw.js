...
const CACHE_NAME = 'smart-asset-manager-v70'; // [RISK 관리 카드에 소유자별 필터 탭 추가] 전체/신랑/
// 와이프 등 소유자별로 위험점수·위험기여도·스트레스테스트를 따로 진단(같은 엔진을 소유자 범위만 좁혀
// 재사용, 시세 캐시 재활용이라 즉시 계산됨) + 카드 안 [⚡ 원클릭 리밸런싱 이동] 버튼이 지금 보던
// 소유자의 리밸런싱 실행 가이드 아코디언을 자동으로 펼치고 스크롤. v69->v70: 이 값을 바꿔야 PWA가
