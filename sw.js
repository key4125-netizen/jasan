...
const CACHE_NAME = 'smart-asset-manager-v135'; // [UI/UX 개선 - 리스크 정밀 진단 카드를 종목 상세 모달로 통합]
// 메인 화면 RISK 관리 카드에서 리스크 감지 종목마다 있던 "🔍 리스크 진단 보기" 펼침 버튼과 그 안의
// 개별 진단 카드(추세/과열도/수급 신호등, 베타, 폭락장 방어 성적표, 계좌 내 비중, 위험 기여도, 권장
// 행동 지침)를 메인 리스트에서 완전히 제거하고, 종목을 클릭했을 때 열리는 상세 모달 하단(차트→보유
// 정보→소유자별 현황 다음)으로 옮겼다(js/10에서 js/08로 이관). 부가 효과로, 예전엔 "리스크 감지" 태그가
// 붙은 종목에만 이 카드가 있었지만 이제는 리스크 지표가 계산된 종목이면 안정/감지 여부와 무관하게 상세
// 모달에서 항상 볼 수 있다. index.html/js/08/js/10이 바뀌었으므로 v134->v135로 올려 PWA가 캐시된 예전
// 버전을 버리고 새로 받아오게 한다.
const APP_SHELL = [
  './',
  './index.html',
