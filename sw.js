...
const CACHE_NAME = 'smart-asset-manager-v129'; // [종목 분석 보고서 - 초보자 친화적 재구성]
// 종목 분석 모달 리포트를 6개 섹션(핵심요약 상태뱃지/주가위치 참고선/기술적분석/거래량·관심도/
// 리스크분석/위험관리 안내)으로 재구성하고 문구를 쉬운 말로 다듬었다. 다만 요청 중 데이터가 없는
// 재무/실적(매출·영업이익·OPM)과 PER·EPS 기반 목표주가 시뮬레이션은 검토 후 제외했다(API가 재무
// 데이터를 제공하지 않아 지어낸 값이 되고, 목표가 제시는 투자자문 경계를 넘음). 가격대별 "관망 구간/
// 관심 구간" 같은 행동 지시형 카드도 빼고, "최근 3개월 최고가/최저가"를 순수 참고정보로만 보여준다
// (recentHigh/recentLow/volumeTrend, js/09). 위험관리 안내는 이 종목의 수치와 무관한 일반 원칙만
// 담아 개인화된 매매 지시가 되지 않게 했다. index.html과 js/09,10이 바뀌었으므로 v128->v129로 올려
