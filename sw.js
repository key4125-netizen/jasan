...
const CACHE_NAME = 'smart-asset-manager-v42'; // [세부내용 버튼 아이콘 통일 - 최종] 총평가손익/일간평가손익
// 카드 모두 [세부내용] 버튼(동일한 크기·아이콘 위치) 옆에 박스 아이콘(trending-up/activity)이 모바일을
// 포함한 모든 화면 폭에서 항상 같이 보이도록 통일했다(이전엔 모바일에서 숨김 처리돼 있었음 - 일간평가손익
// 카드의 원래 구성을 기준으로 두 카드를 맞췄다).
// v41->v42: 이 값을 바꿔야 PWA가 캐시해 둔 예전 index.html을 버리고 새 파일을 다시 받아온다 - 안 바꾸면
