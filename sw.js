...
const CACHE_NAME = 'smart-asset-manager-v91'; // [거래내역 탭 - 전체 거래내역 엑셀 백업 버튼 추가]
// 거래내역 탭에 [거래백업] 버튼을 새로 추가했다 - 지금까지 등록된 거래내역 전체(구분·최초/기간
// 무관)를 `거래내역_백업_YYYYMMDD.xlsx`로 내려받는다(downloadTransactionsBackup). 컬럼 구성을 기존
// [양식다운로드]/[거래등록] 업로드와 완전히 동일하게 맞춰서(구분/일자/소유자/계좌구분/종목명/티커/
// 거래유형/수량/매매단가/통화/적용환율/수수료), 백업 파일을 그대로 다시 [거래등록]에 올려도 100%
// 동일하게 복원된다(실측: 내보낸 파일을 업로드 파서로 되돌려 원본과 완전히 일치함을 확인).
// v90->v91: 이 값을 바꿔야 PWA가 캐시해 둔 예전 index.html을 버리고 새 index.html을 다시 받아온다 - 안
