...
const CACHE_NAME = 'smart-asset-manager-v58'; // [초보자용 포트폴리오 위험 진단 기능 추가]
// 실제 6개월 일별 종가로 베타/VaR/CVaR/Sortino/상관관계/집중도를 계산하는 리스크 엔진과, 그 결과를
// 쉬운 한글 용어("널뛰기 위험", "몰빵 위험도" 등)로 번역해 보여주는 안전 점수 배지 + 접속 시 위험/주의
// 단계에서 뜨는 알림 팝업(1클릭 리밸런싱 이동/상세 카드 이동/오늘 하루 안 보기) + 기존 RISK 관리
// 카드에 위기 시뮬레이션(2020년 코로나 폭락장 재현) 및 종목별 행동 지침 태그를 추가했다.
// v57->v58: 이 값을 바꿔야 PWA가 캐시해 둔 예전 index.html을 버리고 새 파일을 다시 받아온다 - 안 바꾸면
