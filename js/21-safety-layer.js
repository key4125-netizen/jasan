...
// [V1.1 Phase 2 - T-1/T-2 후속] MC 원금이 부동산만 빼고 전부 더하던 것을 일반계좌(비절세·비부동산)
// + 해당 owner 기준으로 좁혔다(computeHouseholdMonteCarloPV) - 절세계좌·부동산·'공동' 자산을 가진
// 사용자는 이 시뮬레이션의 원금이 눈에 보이는 총자산보다 작게 나온다. 이유 없이 줄어든 것처럼
...
function explainAccumulationScopeAlwaysOn() {
  return makeIssue('SAFETY_ACCUMULATION_SCOPE', SAFETY_LEVEL.INFO, 'result', '이 시뮬레이션의 범위 안내',
    '이 시뮬레이션은 자산을 모아가는 적립 단계만 다룹니다. 적립 기간 중 수익률이 오고 가는 순서에 따른 결과 차이는 반영되지만, 은퇴 후 인출 단계에서 발생하는 위험은 이 모델에 포함되어 있지 않습니다. 이 시뮬레이션은 일반계좌의 투자자산을 기준으로 하며, 절세계좌·부동산·공동 자산은 계산에서 제외됩니다.',
    '');
}
