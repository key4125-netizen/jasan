...
function openStockDetailModalReadOnly(ticker, name, sanitized) {
  ...
  pushModalHistoryState();
  lucide.createIcons();
  // [지수류 차트 단위 · §46 MAC-UNIT-01] 매크로 지표는 지표 성격대로 단위를 붙인다 - 금리 %, 원/달러 원,
  // 금 시세 $, 지수 · VIX · 달러인덱스는 단위 없는 지수값(getMacroValueUnit, js/10). "해외 티커"라는
  // 이유만으로 $를 붙이지 않는다(v260까지 원/달러 환율이 "$1,370"처럼 보였다). 종목은 기존대로 통화 기준.
  const valueUnit = macroKey ? getMacroValueUnit(macroKey) : null;
  renderAssetDetailChart({ ticker, name, currency: s.isDomestic === '해외' ? 'USD' : 'KRW', valueUnit });
...
// 단일 평단가만 보여준다.
let assetDetailChartMeta = { name: '', isForeign: false, valueUnit: null, avgPrice: 0, byOwnerAvgPrice: undefined };

// [§46 MAC-UNIT-01] 매크로 지표 값 표기 - valueUnit: 'pct'(금리) · 'krw'(원/달러) · 'usd'(금 시세) · 'point'(지수값).
// valueUnit이 없으면(종목) null을 돌려주고 호출부가 기존 통화 표기를 쓴다.
function formatMacroChartValue(v, valueUnit) {
  if (valueUnit === 'pct') return fmtNum(v, 2) + '%';
  if (valueUnit === 'krw') return fmtNum(v, 1) + '원';
  if (valueUnit === 'usd') return '$' + fmtNum(v, 2);
  if (valueUnit === 'point') return fmtNum(v, 1);
  return null;
}
...
// valueUnit: 매크로 지표 팝업 차트 전용 단위(formatMacroChartValue · §46 MAC-UNIT-01). 종목 상세 · 종목 분석
// 차트에서는 없으며(undefined) 기존처럼 통화 기준($ · ₩)으로 표기한다.
function buildMaLegendHtml(maByPeriod, isForeign, avgPrice, byOwnerAvgPrice, valueUnit) {
  ...
  const maSpans = MA_PERIODS.map((period) => {
    ...
    const valueText = (typeof latest !== 'number') ? '-' : (valueUnit ? formatMacroChartValue(latest, valueUnit) : `${isForeign ? '$' : ''}${fmtNum(latest, isForeign ? 2 : 0)}`);
    return `<span style="color:${maColors[period]}">● MA${period}: ${valueText}</span>`;
  });
  const unit = isForeign ? '$' : '₩';
  const fmtPrice = (v) => valueUnit ? formatMacroChartValue(v, valueUnit) : `${unit}${fmtNum(v, isForeign ? 2 : 0)}`;
  let avgSpan = '';
...
function buildAssetDetailChart(periodKey) {
  ...
  const { name, isForeign, valueUnit, avgPrice, byOwnerAvgPrice } = assetDetailChartMeta;
  ...
  document.getElementById('assetDetailMaLegend').innerHTML = buildMaLegendHtml(maByPeriod, isForeign, avgPrice, byOwnerAvgPrice, valueUnit);
  ...
  charts.assetDetail = new Chart(canvas, {
    ...
    options: {
      ...
      scales: {
        ...
        // [버그 수정 - 지수류에 '$' 오표기 · §46 MAC-UNIT-01] 매크로 지표 팝업(valueUnit 있음)은 지표 성격대로
        // 금리 % · 원/달러 원 · 금 시세 $ · 지수값(단위 없음)으로 표기한다. 세로축은 폭이 좁아 원 · % 대신
        // 숫자만 두고(금리 소수 둘째 자리, 원/달러 정수) 단위는 위 범례가 알려 준다.
        y: {
          ticks: { color: textColor, callback: (v) => valueUnit === 'pct' ? fmtNum(v, 2) : valueUnit === 'krw' ? fmtNum(v, 0) : valueUnit === 'point' ? fmtNum(v, 1) : valueUnit === 'usd' ? ('$' + fmtNum(v, 2)) : (isForeign ? ('$' + fmtNum(v, 2)) : (v >= 10000 ? fmtNum(v / 10000, 2) + '만' : fmtNum(v, 0))) },
...
async function renderAssetDetailChart(asset, avgPriceOverride, byOwnerAvgPriceOverride) {
  ...
  assetDetailChartMeta = { name: asset.name, isForeign: asset.currency === 'USD', valueUnit: asset.valueUnit || null, avgPrice, byOwnerAvgPrice: byOwnerAvgPriceOverride };
