...
function updateRealEstateGuidanceText() {
  ...
  // [Phase 39 - 가구 전체 기준 명시] 리스크 진단은 riskEligibleAssets()에 owner 필터가 없어 처음부터
  // 소유자 구분 없이 가구 전체를 합산해 계산한다(같은 종목을 나눠 보유하면 합쳐진 비중으로 잡힌다).
  // 그동안 이 사실이 코드 주석에만 있어서 사용자가 자기 계좌만의 진단으로 오해할 수 있었다 - 이미
  // 있는 이 한 줄에 덧붙여 알린다(새 카드를 만들지 않는다). 계산에는 아무 영향이 없다.
