...
function updateRealEstateGuidanceText() {
  ...
  /* [PM 지시 2026-09-22] #riskScopeNote는 화면에서 사라졌다 - 같은 설명을 점수 옆 ⓘ
   * 「포트폴리오 위험 안내」 팝업이 전담한다(index.html #portfolioRiskInfoModal).
   * 부동산 보유 여부에 따라 문구를 바꾸던 처리도 함께 필요 없어졌다 - 팝업 문구는
   * "현금 · 채권 · 부동산은 빠집니다"로 두 경우를 모두 맞게 설명한다.
   * 진단 대상 집합(riskEligibleAssets)과 계산은 예전 그대로다. */
...
// 'target'(화면 표시 이름은 "포트폴리오 설정"(v248-1) - 목표 비중 · 적립계획), 'projection'
...
const MODAL_CLOSE_FNS = {
  ...
  positionRoleBreakdownModal: (viaBack) => closePositionRoleBreakdownModal(viaBack),
  ...
  mcInfoModal: (viaBack) => closeMcInfoModal(viaBack),
  projectionAssumptionsModal: (viaBack) => closeProjectionAssumptionsModal(viaBack),
  mcFeeRatesModal: (viaBack) => closeMcFeeRatesModal(viaBack)
};
