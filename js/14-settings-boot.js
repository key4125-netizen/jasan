...
document.getElementById('resetDataBtn').addEventListener('click', () => {
  ...
  // [소유자별 독립 리밸런싱/월적립금 - Option B] js/01 state 기본값과 동일한 구조로 초기화한다(신랑/
...
async function bootApp() {
  ...
  // 반드시 필요하다.
  // [P0 - 과거 이력 자동 소급 중단] 예전엔 시세 갱신이 끝난 뒤 여기서 backfillAllHoldingsDailyPnlHistory()를
  // 불러 기록 없는 과거 날짜를 지금 수량으로 만들고, 기존 과거 cur을 다시 계산해 덮어썼다 - 사실이 아닌 과거를
  // 만들거나 바꾸는 일이라 부팅 경로에서 뺐다(js/11 [P0 HISTORICAL SNAPSHOT INTEGRITY] 주석 참고).
