...
- 판: PHASE 0 초판 · 실행 묶음 B 1~7차 + PM Solution Closure 반영(2026-09-20)
...
| P-7 | SEC 등 요구 연락처의 secret 주입 방식 | 보안 | PHASE 2 | PM_DECISION_REQUIRED | - |
...
- **현재 구현**: **368590 해결** - 발행사 공식 페이지 "미국 나스닥100 지수를 추종하는 환노출형 ETF" → UNHEDGED 확정(A등급), 원장 반영 완료. **360200 미해결** - 한국투자신탁운용 상품 페이지 전체에서 환헤지 문구를 찾지 못했다(일반 위험고지만 존재). [B-4] 360200을 추가 조사했다 - 운용사 두 사이트(ACE · 본사) 모두 환헤지 문구 없음, 투자설명서 버튼은 동작하지 않았다. OpenDART 호출 준비 완료. [B-4 실행] 360200의 정정투자설명서(접수번호 20260213001628 · 최초제출 2020-07-23)를 OpenDART로 확인했지만 표지만 제공돼 환헤지 조항을 읽지 못했다. [B-5] OpenDART로 360200 공시를 특정했으나 본문 미제공. PDF 경로는 도구 한계로 막힘. KIND(HTML 공시)가 새 후보로 확인됨.
...
- **SoT**: §44 44-16 D-06 · 계획서 §19
- **현재 구현**: Alphabet 10-K 표지 실측 — Delaware 설립 · 10-K · 12(b)에 Class A Common Stock(GOOGL)과 Class C Capital Stock(GOOG) 동시 등록 · Nasdaq · ADR 아님. HOME_COMMON_RULE_V2("본국 발행 지분증권, ADR·우선주·ETF 제외")를 적용하면 GOOG는 HOME_COMMON이다.
- **문제**: 측정 완료 - GOOG 판정 {key: NASDAQ, status: RESOLVED}, 상태 분포 RESOLVED 33→34 · UNRESOLVED 15→14, 원장 검증 위반 0건. 규칙 버전 상향(v1→v2)에 대한 PM 승인만 남았다.
...
- **현재 판정**: PM_DECISION_REQUIRED · **단계**: PHASE 3
- **SoT**: 계획서 §22 · §23
- **현재 구현**: KOSPI200 PR/TR 정의는 A등급 확정, 일별 시계열만 없다. PM Solution Closure에서 "ETF를 지수 proxy로 쓰는 안"을 실제로 검토한 결과 **무의미함을 확인**했다 — proxy 후보가 베타를 구하려는 ETF 자신이라 베타가 정의상 1이 되고, 현재 원장에 KOSPI200을 기준으로 삼는 다른 종목이 없다.
- **문제**: proxy 대안은 권고하지 않는다(정보량 0). 실효 경로는 (가) 또는 (나)이며, 둘 다 외부 조치가 선행된다.
...
- **현재 구현**: 원화 환산 낙폭도 같은 정의로 계산했다(지수 수준 × 같은 날짜 H.10 · 6,654일 일치). 예: 나스닥 종합 2020 -27.65% · 2022 -31.37%, S&P 500 2020 -29.86% · 2022 -17.55%.
...
- **현재 구현**: 저장소 cloudflare-worker-kis-proxy.js 실측 - ① CORS가 Access-Control-Allow-Origin:* 로 전면 개방 ② CLIENT_SHARED_SECRET 미등록 시 인증 검사가 통째로 생략되는 fail-open ③ 요청 수 제한 없음. 수정안(Origin 허용목록 · fail-closed · KV rate limit · 오류 최소노출)과 Cloudflare Dashboard 작업 순서를 PM_SOLUTION_CLOSURE.md §9에 확정했다.
- **문제**: 코드 수정 자체는 즉시 가능하나, 비밀값 회전과 Worker 재배포는 사용자(대시보드) 작업이다. 공개 PWA의 X-App-Secret은 본질적으로 공개값이므로 실질 방어선은 Origin 허용목록 + rate limit임을 문서화했다.
...
- **현재 판정**: PM_DECISION_REQUIRED · **단계**: PHASE 2
- **SoT**: 계획서 §9
- **현재 구현**: 실측 확인: 연락처 없는 User-Agent로는 SEC가 403으로 막는다(www.sec.gov · data.sec.gov 모두). SEC 공식 문서가 UA에 연락 이메일을 요구한다.
- **문제**: D-1(미국 개별주 20종목)을 진행하려면 SEC 요청 헤더에 쓸 연락처가 필요하다. 사용자 개인 이메일을 외부 서비스에 보내는 것은 사용자의 명시적 허가 없이 하지 않는다.
...
- **현재 구현**: CMA 2026Q2(VERIFIED · 기준일 2026-03-31)와 현행 Q1의 차이를 측정했다. 앱 사용 4개 자산군 ER 모두 상승 · σ 상승(한국주식 27.9→29.4가 최대). 동일 seed 20260101 · 2,000회 · 20년 · 원장 49건 MC: P10 -1.97% · P50 +1.52% · P90 +4.77% · mean +3.84% · μ는 변화 없음(Return Key에서 오므로 CMA와 무관 - 정책 경계 정상 작동 확인).
- **문제**: 영향이 정량화됐다. 권고는 (나) Q2 활성화 - 기준일이 더 최신이고 μ에 영향이 없으며 변화가 σ·상관에 한정된다. BOND-4와 동시 적용 금지(순차 적용 + 각각 전후 측정).
...
- **현재 구현**: 조사 · 설계 완료. 산출물 3종 - BOND_DECISION_PACKAGE.md(BOND-1~6 결정표 + 권고) · BOND_DOMAIN_DESIGN.md(4계층 분리 · 스키마 · 가격 의미 · ISIN 조회 UX · 현금흐름 · 수익률 · Risk · MC · 외화 · 보안 · 충돌 점검 · 구현 순서) · BOND_DATA_SOURCE_MATRIX.md(원천 8종 · 라이선스 · 자동화 판정). 2026-09-20 PM Solution Closure에서 BOND-1~6 전부에 채택 권고안을 확정했다(PM_SOLUTION_CLOSURE.md). BOND-3 · 4 · 6은 해법이 닫혔고(SOLVED WITH CONSTRAINT), 1 · 2 · 5는 PM 선택만 남았다.
- **문제**: BOND-4의 "국내채권 CMA 없음"은 해소됐다 — JPM-LTCMA-2026-KRW에 Korean Government/Corporate Bonds가 이미 있다. 남은 것은 해당 데이터셋의 역할 승격(§37) 승인이다.
...
- **문제**: 사실이 바뀌어도 사람이 발견하지 않으면 원장이 낡는다. 반대로 자동 적용이 과하면 정책을 조용히 바꾼다. **추가 제약(2026-09-20 조사)**: 주요 원천의 이용조건이 자동 수집 · 재배포를 제한한다(Yahoo robots 전면 Disallow · 공공데이터 4유형 재배포 금지). 자동화 설계는 이 제약 안에서만 가능하다.
...
- **현재 구현**: Bond Domain 설계가 기존 정책 3곳과 정면으로 만난다. ① §7: 채권 · 현금 σ=0 ↔ BOND-4 CMA σ 사용 ② RISK_ELIGIBLE_CATEGORIES=[주식,ETF] ↔ BOND-2 직접채권 편입 ③ §44 제10조 포트폴리오 베타 전부-또는-무 ↔ BOND-5 부분 집계. PM Solution Closure에서 3건 모두에 대해 SoT 변경안(§7-1 Risk 대상 · §7-2 제10조 베타 · §7-3 MC 채권 자산군)을 CURRENT/PROBLEM/PROPOSED/WHY/IMPACT/REGRESSION/ROLLBACK 형식으로 작성했다.
