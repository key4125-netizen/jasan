...
| **위험 지표 전반**(변동성 · 베타 · VaR · CVaR · 최대낙폭 · 위험점수) | 가격 조회를 1년 → 3년으로 늘리고 지표마다 필요한 기간을 쓰도록 했다(§44 제7조 이행 · C-3) | **최대낙폭이 커진다** — 1년 창은 직전 1년에 큰 하락이 없으면 실제보다 작게 보였다(실측 예: −13.8% → −33.4%). 변동성 · 베타도 2년 기준으로 바뀐다 |
| 스트레스 손실 추정 | 낙폭 상수를 실측으로 다시 맞추고(코스닥 2020 −33.0 → −38.15) 나스닥 종합을 추가, 원화 환산 지수에는 원화 기준 낙폭을 곱한다(D-7 · D-9) | 코스닥 보유 시 추정 손실이 커진다. 지금까지 "계산할 수 없음"이던 경우(나스닥 종합 · 원화 환산 ETF)가 값으로 나온다 |
| 종목별 위험 기여도 | 음수를 0으로 깎지 않는다(F-7) | 포트폴리오와 반대로 움직인 종목이 **음수(−N%)** 로 표시될 수 있다 — 위험을 낮췄다는 뜻이다 |
| 엑셀 가져오기 | 고치지 않고 다시 올린 파일이 자산군을 "사용자 확정"으로 바꾸지 않는다(G-3) | 눈에 보이는 변화는 없다. 자동 분류 개선이 계속 닿는다 |
| 종목 분석 팝업 | 기준 지수를 §44 제10조 경로로 통일(G-1) | 화면에 쓰이지 않던 내부 참고값이라 **사용자 화면 변화 없음** |
