...
- 그 대가(trade-off)를 명시한다: 완전히 올바른 모델이 우연히 탈락할 확률이 증가한다. 필수 Horizon 2개(1M·3M) 기준 독립 가정 상한은 1−(0.95)² = **약 9.75%** 이며, 두 Horizon이 같은 데이터를 공유하므로 실제 값은 그보다 낮다. 정확한 값은 제63조 MC-NULL에서 산출해 Gate Result에 기록한다.
