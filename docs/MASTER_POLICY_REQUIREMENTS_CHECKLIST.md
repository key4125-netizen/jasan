...
- **현재 production: v242**(2026-09-14) — v238 과거 이력 무결성(§25) · v239 신규 시작 안정화(§26) · v240 일별 이력 복구 제거 · 클라우드 데이터 초기화(§27) · v241 총자산 추이 Daily Valuation(§28) · v242 일별 손익 추이 Daily Valuation · 두 팝업 기간 통일(§29). 아래는 당시 기록이다 — **이전 기록: 현재 production: v237**(2026-09-13). v236 = 일별 이력 복구 **안전성 수정의 코드 배포**(§22), v237 = 매크로 브리핑 UX 단순화 **RELEASE ACCEPTED**(§23). **실제 사용자 데이터에 대한 Recovery Apply는 BLOCKED**이며, v236 Recovery 문제는 **해결 완료가 아니다**. 현재 상태 요약은 §24.
